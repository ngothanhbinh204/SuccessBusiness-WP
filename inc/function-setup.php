...
function canhcam_style()
{
	...
	// Conditional styles
	if (is_page_template('template-faq.php') || is_page_template('page-templates/template-faq.php')) {
		wp_enqueue_style('faq-filter-style', THEME_URI . '/styles/faq-filter.css', array(), GENERATE_VERSION);
	}
	
	if (is_search()) {
		wp_enqueue_style('search-style', THEME_URI . '/styles/search.css', array(), GENERATE_VERSION);
	}
	...
	if (class_exists('CanhCam_Licsence_Class')) {
		...
		if (!$my_license->isDateExpiration()) {
			if (stripos($_SERVER['HTTP_USER_AGENT'], 'Chrome-Lighthouse') === false) {
				...
				// Conditional scripts
				if (is_page_template('template-faq.php') || is_page_template('page-templates/template-faq.php')) {
					wp_enqueue_script('faq-filter', THEME_URI . '/scripts/faq-filter.js', array('jquery'), GENERATE_VERSION, true);
				}
			}
		}
	}
